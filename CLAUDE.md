...
- [ ] **Star career: give `energy` a real New-Star-Soccer-style gameplay effect** (Aug 2026) — `career.energy` currently exists (0-100, drains from playing/training, regenerates on week rollover/rest — see `lib/star/week.ts`) but nothing meaningfully gates on it. The user's stated design, to build later: (1) a minimum energy required to START a match at all; (2) a minimum energy required to bring on a SUBSTITUTE; (3) energy regenerates when skipping to the end of the week (this part already exists — `startNewWeek()`/`WEEK_RECOVERY` in `lib/star/week.ts`). As of this note, the energy bar (dashboard top), the NRG-drinks shop, and training's energy cost/gate have all been deliberately REMOVED from the UI (see "19 August 2026" session below for exact revert points) so energy currently just sits there unused between matches — `career.energy` itself, its week-rollover regen, `LifeScreen.tsx`'s Rest button, and the relationship minigames' energy cost were all left untouched. When implementing: the live in-match energy meter (`energyRef` in `CanvasMatch.tsx`, seeded from `career.energy` at kickoff) and `hiddenMatch.ts`'s `HiddenMatchInputs.energy` (which already softly affects chance-involvement rate mid-match) are a SEPARATE, already-working mechanism — don't confuse the two when wiring the new gates in.
...
- **Energy pulled from the UI, not the data model** — see the "What Needs Improvement" entry above for the full plan. `career.energy` still exists and still drains/regenerates exactly as before (`lib/star/week.ts`), but nothing currently gates on it: training no longer costs or checks it (`SkillsScreen.tsx`, `handleTrainingComplete` in `app/star-dev/page.tsx`), the dashboard's top energy bar is gone (`DashboardShell.tsx`), and the NRG-drinks shop was removed outright rather than hidden — the Home page card, its buy button, `Shop.tsx`'s whole `"nrg"` tab, and the `nrgDrinks`/`NRG_DRINKS` data (`lib/star/types.ts`, `careerFlow.ts`, `shopData.ts`) are all gone. `LifeScreen.tsx`'s Rest button and the relationship minigames' own energy cost were left untouched — out of scope for this pass.
