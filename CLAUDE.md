...
| `fc27_clone_premier_league.sql` | **PENDING** (new) | Clones every FC 26 Premier League player into `fifa_year = 2027` a year older, so the 2026/27 season can be built by hand before FC 27 exists. **Run the four steps ONE AT A TIME** — the SQL Editor shows only the last statement's result, so a step that found nothing looks identical to one that worked. Step 1 says whether there is anything to copy; step 2 RETURNs the rows it wrote. `ON CONFLICT DO NOTHING`, so re-running never undoes your edits. |
