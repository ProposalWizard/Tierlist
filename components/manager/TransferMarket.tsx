  ...
  return (
    <div className="max-w-2xl mx-auto p-4 pb-20">
      ...
      <div className="text-center mb-6">
        ...
        <p className="text-white text-sm mt-1">Choose one player per position</p>
      </div>

      {/* Budget bar */}
      <div className="bg-gray-900 rounded-xl p-4 mb-5 border border-gray-800/50">
        <div className="flex items-center justify-between mb-2">
          <span className="text-xs font-bold text-white uppercase tracking-widest">Budget Remaining</span>
        ...
        <div className="flex justify-between mt-1">
          ...
          <span className="text-[10px] text-white">£{BUDGET_START}M</span>
        </div>
      </div>
                ...
                >
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-xs text-white mt-0.5">{opt.club} · {opt.year} · Age {opt.player.age}</div>
                      {/* Key stats */}
                      {opt.player.attrs && (
                        <div className="flex gap-3 mt-2">
                          {isSub(currentSlot) || slotLabel(currentSlot) === "GK" ? (
                            opt.player.attrs.gkReflexes > 0 ? (
                              <>
                                <span className="text-[10px] text-white">DIV <span className="text-white font-bold">{opt.player.attrs.gkDiving}</span></span>
                                <span className="text-[10px] text-white">REF <span className="text-white font-bold">{opt.player.attrs.gkReflexes}</span></span>
                              </>
                            ) : (
                              <>
                                <span className="text-[10px] text-white">PAC <span className="text-white font-bold">{opt.player.attrs.pace}</span></span>
                                <span className="text-[10px] text-white">SHO <span className="text-white font-bold">{opt.player.attrs.shooting}</span></span>
                                <span className="text-[10px] text-white">PAS <span className="text-white font-bold">{opt.player.attrs.passing}</span></span>
                              </>
                            )
                          ) : (
                            <>
                              ...
                              <span className="text-[10px] text-white">SHO <span className="text-white font-bold">{opt.player.attrs.shooting}</span></span>
                              <span className="text-[10px] text-white">PAS <span className="text-white font-bold">{opt.player.attrs.passing}</span></span>
                              <span className="text-[10px] text-white">DEF <span className="text-white font-bold">{opt.player.attrs.defending}</span></span>
                            </>
                          )}
                        </div>
                      )}
        ...
        {currentState?.picked && (
          <div className="bg-emerald-900/20 border border-emerald-700/30 rounded-xl p-4 flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-xs text-white">{currentState.picked.overall} OVR · {currentState.picked.clubYear}</div>
            </div>
          </div>
        )}
      </div>
      ...
      {squad.length > 0 && (
        <div className="bg-gray-900 rounded-xl p-4 border border-gray-800/50">
          <h3 className="text-[10px] font-bold tracking-widest text-white uppercase mb-2">Squad So Far</h3>
