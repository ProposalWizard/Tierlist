...
export default function DraftResult({ players, onNewRun, onPlayNextSeason, seasonNumber = 1, previousResult, allSeasonResults, formationName, isSignedIn = false, preComputedSeason, roomPlayers, roomCode, allRoomPlayerSeasons, mode = "normal", revealStartTime, speedMultiplier = 1, playerTeamName, isFinalSeason = false }: Props) {
  ...
  return (
    <div className="max-w-2xl mx-auto p-4 pb-20">
      ...
      {/* Season N+1 — big prominent button; in multiplayer wait for all players to finish */}
      {onPlayNextSeason && season.actualFinish < 18 && (() => {
        const allSimulated = !roomPlayers || roomPlayers.length === 0
          || roomPlayers.filter(p => p.status !== "out").every(p => p.status === "simulated");
        return allSimulated ? (
          <button
          ...
          <div className="w-full py-4 mb-4 bg-gray-800/50 border border-gray-700/40 rounded-xl text-sm text-white/60 text-center flex items-center justify-center gap-2">
            <svg className="w-4 h-4 animate-spin opacity-60" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" /><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" /></svg>
