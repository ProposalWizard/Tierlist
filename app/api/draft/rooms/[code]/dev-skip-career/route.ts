...
import type { DraftPlayer, SharedSeasonInput, LeagueTeam, PlayerAttributes, SeasonResult } from "@/lib/seasonSimulator";
...
export async function POST(
  ...
  try {
    for (let seasonNumber = startSeason; seasonNumber <= MAX_SEASONS; seasonNumber++) {
      ...
      if (firstResult?.leagueTable) {
        previousLeagueTable = firstResult.leagueTable.map(t => ({
          name: t.name,
          played: t.played,
          won: t.won,
          drawn: t.drawn,
    ...
    // Fetch final room players for display
    ...
    return new Response(`Career simulation failed: ${msg}`, { status: 500 });
  }
}
