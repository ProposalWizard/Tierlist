...
export const ADMIN_GUIDES = {
  ...
  "/star-highlights-dev": {
    ...
    buttons: [
      ...
      },
      {
        ...
        items: [
          ...
          ["Save this fix / Discard", "Appear after a drag. Save puts the fixed picture in the shared list; Discard throws the drags away."],
          ["Saved — revert to the generated chance", "Deletes your saved fix from the shared list."],
    ...
    saving: [
      ...
      "Save this fix: " + SCENARIO_SAVE_SHARED + " It lands in the gallery as a simulated card of its type.",
      SCENARIO_NOT_IN_GAME_UNTIL_COMMIT,
      "Tune corrections go to the shared table star_scenario_corrections.",
    ],
    commit: [
      "Nothing here commits. To put a saved fix into the game, go to Scenario Gallery → Tuning & Commit.",
      "Delete is the exception: it also removes a saved chance from the code (needs GITHUB_TOKEN in Vercel).",
    ],
    inGame: [
      "The chances are generated exactly as in a real /star-dev match, so what you see here is what players get.",
      "Your fixes reach real matches only once they're committed from the gallery.",
    ],
